...
# compose-up-native is a one-release alias of the default native stack.
COMPOSE_NATIVE_EPHEMERAL  := $(COMPOSE_EPHEMERAL)
COMPOSE_NATIVE_PERSISTENT := $(COMPOSE_PERSISTENT)
COMPOSE_NATIVE            := $(COMPOSE_EPHEMERAL)
...
COMPOSE_389DS_EPHEMERAL  := docker compose -f deploy/compose/compose.yaml -f deploy/compose/compose.389ds.yaml -f deploy/compose/compose.389ds-ephemeral.yaml -p labldap
COMPOSE_389DS_PERSISTENT := docker compose -f deploy/compose/compose.yaml -f deploy/compose/compose.389ds.yaml -f deploy/compose/compose.persistent.yaml -p labldap
COMPOSE_389DS            := $(COMPOSE_389DS_EPHEMERAL)
COMPOSE_389DS_SCENARIO            := deploy/compose/scenario.389ds.yaml
COMPOSE_389DS_PERSISTENT_SCENARIO := deploy/compose/scenario.389ds-persistent.yaml
...
help:
	@printf '%s\n' \
		...
		'  compose-up         native labldapd; ephemeral 2GiB tmpfs /data; bootstrap → control' \
		'  compose-up-persistent  native engine; named volume /data' \
		'  compose-down       stop the Compose project' \
		'  compose-reset      operator hard reset: down -v then compose-up (not REST/MCP)' \
		'  compose-up-389ds   389 oracle/rollback; ephemeral tmpfs; publish instance CA' \
		'  compose-up-389ds-persistent  389 oracle; named volume; dsctl tls import lab CA' \
		'  compose-up-native  alias of compose-up (one release)' \
		'  compose-up-native-persistent  alias of compose-up-persistent' \
		'  compose-down-native   alias of compose-down' \
		'  compose-reset-native  alias of compose-reset' \
...
# Default stack is native labldapd. labldapd self-applies the engine plan
# and serves the lab CA certificate directly, so there is no setuptls
# publish/import (dsctl) step. LABLDAP_TLS_CA is the lab CA itself.
...
compose-down:
	...
	-$(COMPOSE_389DS_EPHEMERAL) down --remove-orphans
	-$(COMPOSE_389DS_PERSISTENT) down --remove-orphans

compose-reset:
	-$(COMPOSE) down --remove-orphans -v
	-$(COMPOSE_PERSISTENT) down --remove-orphans -v
	-$(COMPOSE_389DS_EPHEMERAL) down --remove-orphans -v
	-$(COMPOSE_389DS_PERSISTENT) down --remove-orphans -v
	$(MAKE) compose-up

# One-release aliases: native is now the default compose-up.
...
# 389 Directory Server oracle / rollback (explicit engine: 389ds).
compose-up-389ds: image image-bootstrap compose-preflight compose-secrets setup-tls
	LABLDAP_SCENARIO_FILE=$(COMPOSE_389DS_SCENARIO) LABLDAP_TLS_CA=$(COMPOSE_TLS)/instance-ca.crt \
		$(COMPOSE_389DS) up -d --wait --remove-orphans directory
	$(GO) run ./tools/setuptls publish --out $(COMPOSE_TLS)/instance-ca.crt --project labldap \
		-f deploy/compose/compose.yaml -f deploy/compose/compose.389ds.yaml -f deploy/compose/compose.389ds-ephemeral.yaml
	LABLDAP_SCENARIO_FILE=$(COMPOSE_389DS_SCENARIO) LABLDAP_TLS_CA=$(COMPOSE_TLS)/instance-ca.crt \
		$(COMPOSE_389DS) up -d --no-deps --force-recreate secret-prep
	LABLDAP_SCENARIO_FILE=$(COMPOSE_389DS_SCENARIO) LABLDAP_TLS_CA=$(COMPOSE_TLS)/instance-ca.crt \
		$(COMPOSE_389DS) wait secret-prep
	LABLDAP_SCENARIO_FILE=$(COMPOSE_389DS_SCENARIO) LABLDAP_TLS_CA=$(COMPOSE_TLS)/instance-ca.crt \
		$(COMPOSE_389DS) up -d --wait --remove-orphans --force-recreate control

compose-up-389ds-persistent: image image-bootstrap compose-preflight compose-secrets setup-tls
	LABLDAP_SCENARIO_FILE=$(COMPOSE_389DS_PERSISTENT_SCENARIO) \
	LABLDAP_TLS_CA=$(COMPOSE_TLS)/ca.crt \
		$(COMPOSE_389DS_PERSISTENT) up -d --wait --remove-orphans directory
	$(GO) run ./tools/setuptls import --dir $(COMPOSE_TLS) --project labldap \
		-f deploy/compose/compose.yaml -f deploy/compose/compose.389ds.yaml -f deploy/compose/compose.persistent.yaml
	LABLDAP_SCENARIO_FILE=$(COMPOSE_389DS_PERSISTENT_SCENARIO) \
	LABLDAP_TLS_CA=$(COMPOSE_TLS)/ca.crt \
		$(COMPOSE_389DS_PERSISTENT) up -d --no-deps --force-recreate secret-prep
	LABLDAP_SCENARIO_FILE=$(COMPOSE_389DS_PERSISTENT_SCENARIO) \
	LABLDAP_TLS_CA=$(COMPOSE_TLS)/ca.crt \
		$(COMPOSE_389DS_PERSISTENT) wait secret-prep
	LABLDAP_SCENARIO_FILE=$(COMPOSE_389DS_PERSISTENT_SCENARIO) \
	LABLDAP_TLS_CA=$(COMPOSE_TLS)/ca.crt \
		$(COMPOSE_389DS_PERSISTENT) up -d --wait --remove-orphans --force-recreate control
