...
| OD-005 | Agent default | Initial supported host architectures. | Support `linux/amd64` and `linux/arm64`. Document any upstream 389 DS image limitation discovered by CI. | T-108 through T-120. |
...
| OD-012 | Verification | Runtime metadata storage in LDAP. | Prototype a protected metadata entry beneath the managed suffix using namespaced attributes. Define and register a private OID schema before stable release if standard attributes cannot safely represent baseline revision and ownership metadata. | T-021, T-038, and ADR checkpoint before release. |
