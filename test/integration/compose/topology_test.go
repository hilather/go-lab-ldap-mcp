...
func requireCompose(t *testing.T) string {
	...
	// D2/D4). Default compose.yaml is native; this stack uses compose.389ds.yaml.
...
func overlayFiles(root string, persistent bool) []string {
	files := []string{
		filepath.Join(root, "deploy/compose/compose.yaml"),
		filepath.Join(root, "deploy/compose/compose.389ds.yaml"),
	}
	if persistent {
