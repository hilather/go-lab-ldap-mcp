...
| `controlplane.go` | always | Contract §5 rule 2: the same engines, through `internal/app` + `ds389.Runtime` (no HTTP). Create/Get/search visibility (C13), bind-test (C3), password policy (C4/C11), If-Match (C9), memberOf + leftover `nsmemberof` (C7/D26). |
